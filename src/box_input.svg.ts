...
export function get_svg_box_input (
    { len, pos_x, pos_y, box_px_width, box_px_height, box_px_border, box_padding, current_text }: SvgBoxInputElement,
    custom_font?: { [key: string]: string },
    ignore_text: boolean = false,
): string {
    let acc = '';
    for ( let i = 0; i < len; i++ ) {
        acc += get_svg_box( {
            ...
            body: !ignore_text && current_text && i < current_text.length ?
                ( custom_font && custom_font[current_text[i]] ? custom_font[current_text[i]] :
